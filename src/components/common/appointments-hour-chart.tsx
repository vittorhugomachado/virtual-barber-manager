  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      const closed =
        !hoursData ||
        hoursData.length === 0 ||
        hoursData.every(row => !row.is_open);
      ...
      for (const appointment of aptsData ?? []) {
        const hour = new Date(appointment.starts_at).getUTCHours();
        if (appointment.status === "completed") {
          buckets[hour].concluido++;
        } else if (
          appointment.status === "cancelled_by_customer" ||
          appointment.status === "cancelled_by_barbershop"
        ) {
          buckets[hour].cancelado++;
        } else {
          buckets[hour].agendado++;
        }
      }
